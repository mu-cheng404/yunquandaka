...
async function send_apply_message(user_id, receiver_id, flock_id) {
  //定义
  let [sender_id, content, time, url, hasRead] = [user_id, '', utils.formatTime(new Date()), `../flock/flock?id=${flock_id}`, 0]
  //生成内容
  let sql = `select user.nickName as user_name, flock.name as flock_name from user, flock where user.id = ${user_id} and flock.id = ${flock_id}`
  ...
  let sql1 = `insert into message(sender_id, receiver_id, flock_id,content, time, url,hasRead,type) values(${sender_id}, ${receiver_id},${flock_id}, "${content}", '${time}', '${url}',${hasRead},1)`
...
async function send_invitation_message(sender_id, receiver_id, flock_id, task_id) {
  ...
  await sql.message_insert(sender_id, receiver_id, content, time, url, 2)
  return true
}
/**
 * 生成周期报告消息内容
